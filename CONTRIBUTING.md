...
```bash
# Build the extension (from leekscript-vscode/)
npm install && npm run compile && npx vsce package --no-dependencies
```

- **F5** in VS Code (with `leekscript-vscode` as the opened folder) launches the Development Host with the extension loaded.
